...
export function getStartStopIndices({
  ...
  while (currentIndex < maxIndex) {
    const bounds = cachedBounds.get(currentIndex)
    if (bounds.scrollOffset + bounds.size >= containerScrollOffset + containerSize) break
    currentIndex++
  }
  ...
  return { startIndexVisible, stopIndexVisible, startIndexOverscan, stopIndexOverscan }
}
